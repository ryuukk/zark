...
#if defined(textureArrayFlag) && defined(textureFlag)
...
void main() {
	...
	//float h = u_waveAmp * sin(u_waveFreq * a_position.y + u_time + a_position.x + a_position.z);
	...
	//region LIGHT
		...
		v_shadowMapUv.z = min(v_shadowMapUv.z, 0.998);
			...
			//vec3 normal = normalize(transpose(mat3(inverse(u_worldTrans))) * a_normal);
		#endif
		v_normal = normal;
	#endif // normalFlag
