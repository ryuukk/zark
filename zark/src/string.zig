// from: https://github.com/clownpriest/strings/blob/master/src/strings.zig
...
pub const string = struct {
    ...
    pub fn init_a(str: []const u8, alloc: *Allocator) !string {
        var buf = try alloc.alloc(u8, str.len);
        for (str) |c, i| {
            buf[i] = c;
        }
        return string {
